...
ESX.RegisterServerCallback("conde_inventory:getPlayerInventory", function(source, cb, target)
...
AddEventHandler(
	...
	function(from, target, type, itemName, itemCount)
		...
		if type == "item_standard" then
			...
			--		TriggerEvent("tqrp_base:serverlog", "[TRADEITEM]: " .. GetPlayerName(_source) .. " ("..itemName .. ") x" .. itemCount .. " to " .. GetPlayerName(target), _source, GetCurrentResourceName())
                    ...
                    TriggerClientEvent('mythic_notify:client:SendAlert', _source , { type = 'error', text = 'El destinatario no tiene espacio en el inventario!' })
                end
            end
	...
	--			TriggerEvent("tqrp_base:serverlog", "[TRADEWEAPON]: " .. GetPlayerName(_source) .. " ("..itemName .. ") x" .. itemCount .. " to " .. GetPlayerName(target), _source, GetCurrentResourceName())
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = '¡El destinatario ya tiene esta arma!' })
			end
		end
	end
)
	...
	if type == "item_standard" then
		...
		if xPlayer.canCarryItem(item, count) then
			...
			--			TriggerEvent("tqrp_base:serverlog", "[BUYITEM]: ("..item .. ") x" .. count .. " $("..totalPrice..")", _source, GetCurrentResourceName())
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No tienes dinero suficiente' })
					end
				end
			end
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No puedes cargar nada más en el inventario!' })
		end
end
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				--		TriggerEvent("tqrp_base:serverlog", "[BUYAMMO]: ("..list[i].weaponhash .. ") x" .. ammo .. " $("..totalPrice..")", _source, GetCurrentResourceName())
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'No tienes el arma para este tipo de munición' })
				end
			end
		end
	end
