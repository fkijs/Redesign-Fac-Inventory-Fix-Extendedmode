...
AddEventHandler('conde-inventoryhud:steal', function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_enter', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 or IsEntityPlayingAnim(searchPlayerPed, "mp_arresting", "idle", 3) or IsEntityPlayingAnim(searchPlayerPed, "mp_arrest_paired", "crook_p2_back_right", 3) then
        ...
        exports['mythic_notify']:SendAlert('error', 'wei')
    end
end)
