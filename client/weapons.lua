...
AddEventHandler('tqrp_inventoryhud:useAttach', function(attach)
    ...
    if currentWeapon ~= nil then
		...
		if weapons[tostring( hash )] ~= nil and weapons[tostring( hash )][attach] ~= nil and not hasAttach then
            ...
            exports['b1g_notify']:Notify('error', 'Este accesorio no es compatible o ya está equipado.')
        end
    else
        exports['b1g_notify']:Notify('error', 'Ningún arma seleccionada.')
    end
end)
...
RegisterCommand("desequipar", function(source, args, rawCommand)
    if currentWeapon ~= nil then
                ...
                if currentWepAttachs[i] == attach then
                    ESX.TriggerServerCallback('tqrp_inventoryhud:addPlayerItem', function(cb)
                        if cb then
                            ...
                            exports['b1g_notify']:Notify('error', 'Espacio insuficiente.')
                        end          
                    end, currentWepAttachs[i], 1)
                    return
                end
            end
            exports['b1g_notify']:Notify('error', 'Esta pistola no tiene este accesorio.')
		else
			for i = 1, #currentWepAttachs do
        ...
        exports['b1g_notify']:Notify('error', 'No tienes un arma en tu mano.')
    end
end)
