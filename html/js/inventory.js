...
window.addEventListener("message", function (event) {
    ...
    //    $("#noSecondInventoryMessage").html(invLocale.secondInventoryNotAvailable);
        ...
        $.post("http://fac_inventory/GiveItem", JSON.stringify({
            player: player,
...
function closeInventory() {
    $(".ui").fadeOut();
    $.post("http://fac_inventory/NUIFocusOff", JSON.stringify({}));
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                $.post("http://fac_inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1,
                }));
            }
...
$(document).ready(function () {
        ...
        if (type === "normal" && itemInventory === "main" && itemData.usable) {
            if (itemData.name.includes("WEAPON_")) {
                if (itemInventory === "fast") 
                {
                    ...
                    $.post("http://fac_inventory/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
                
            } else {
                $.post("http://fac_inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
                closeInventory();
            }
    ...
    $(document).on('contextmenu', '.item', function (e) {
        ...
        if(e.shiftKey) {
            if (itemInventory === "second") {
                if (type === "trunk") {
                    $.post("http://fac_inventory/TakeFromTrunk", JSON.stringify({
    
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("http://fac_inventory/TakeFromProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("http://fac_inventory/TakeFromVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("http://fac_inventory/TakeFromPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                    ...
                    $.post("http://fac_inventory/TakeFromShop", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motels") {
                    $.post("http://fac_inventory/TakeFromMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motelsbed") {
                    $.post("http://fac_inventory/TakeFromMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("http://fac_inventory/TakeFromGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
                ...
                if (type === "trunk") {
                    $.post("http://fac_inventory/PutIntoTrunk", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("http://fac_inventory/PutIntoProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("http://fac_inventory/PutIntoVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("http://fac_inventory/PutIntoPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "motels") {
                    $.post("http://fac_inventory/PutIntoMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }))
                } else if (type === "motelsbed") {
                    $.post("http://fac_inventory/PutIntoMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("http://fac_inventory/PutIntoGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://fac_inventory/DropItem", JSON.stringify({
